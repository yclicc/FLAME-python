...
def get_nearest(distances, n_neighbors, n_samples):
...
def get_densities(distances, nearest, n_samples):
	# Make a numpy arange for iteration purposes.
	sample_range = np.arange(n_samples)[:, None]
	nearest_np = np.array([list(s) for s in nearest])
	n_shortest_distances = distances[nearest_np, sample_range]
	local_distance_sums = n_shortest_distances.sum(axis=1)
	largest_local_sum = local_distance_sums.max(axis=0)
	densities = largest_local_sum / local_distance_sums
	return densities

def get_supports(densities, nearest_with_ties, thd, n_samples):
	density_sum = densities.sum()
	density_mean = density_sum / n_samples
	density_sum2 = (densities * densities).sum()
	thd = density_mean + thd * sqrt( density_sum2 / n_samples - density_mean * density_mean)
	csos = []
	outliers = []
	remaining = []
	for i in range(n_samples):
...
def get_weights(distances, nearest_with_ties, fixed, n_samples, verbose=0):
	...
	for i in range(n_samples):
		...
		if verbose: print "Assigned weights {0}.".format(i)
	weights = weights.tocsr()
	weights = normalize(weights, norm='l1', axis=1, copy=False)
	return weights

def get_starting_membership(csos, outliers, fixed, n_samples):
	M = len(csos) + 1
...
def flame(X, metric='euclidean', cluster_neighbors=5, iteration_neighbors=5, max_iter=np.inf, eps=1e-10, thd=-2, verbose=0):
	"""
	...
	if sparse.issparse(X):
		raise TypeError("FLAME does not yet support sparse data.")
	...
	nearest, nearest_with_ties = get_nearest(distances, cluster_neighbors, n_samples)
	if verbose: print "Got distances and nearest."
	densities = get_densities(distances, nearest, n_samples)
	if verbose: print "Got densities."
	csos, outliers, remaining = get_supports(densities, nearest_with_ties, thd, n_samples)
	if verbose: print "Got suppports."
	if verbose: print "There are {0} clusters and {1} outliers.".format(len(csos), len(outliers))
	fixed = set(csos) | set(outliers)
	_, nearest_with_ties_for_iteration = get_nearest(distances, iteration_neighbors, n_samples)
	weights = get_weights(distances, nearest_with_ties_for_iteration, fixed, n_samples, verbose=verbose)
	if verbose: print "Got weights."
	membership_proba = get_starting_membership(csos, outliers, fixed, n_samples)
	if verbose: print "Got starting memberships."
	i = 0
	while i < max_iter:
		...
		i += 1
		if verbose: print "Done iteration {0}.".format(i)
		if delta < eps:
			break
...
class FLAME(BaseEstimator, ClusterMixin):
	...
	def fit(self, X):
		self.membership_proba_, self.labels_, self.csos_, self.outliers_, self.densities_ = \
			flame(X, self.metric, self.cluster_neighbors, self.iteration_neighbors, self.max_iter, self.eps, self.thd, self.verbose)
	...
	def fit_predict_proba(self, X):
		return self.fit(X).membership_proba_

if __name__== "__main__":
	X = np.array(
		[[0, 0, 0], [1.1, 0, 0], [0, 0.8, 0], [0, 0, 1.3], [10, 10, 10], [11.1, 10, 10], [10, 10.8, 10], [10, 11, 12]])
	membership = flame(X, cluster_neighbors=3, iteration_neighbors=3)
	print membership
